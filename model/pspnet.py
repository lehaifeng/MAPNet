...
def pyramid_pooling_block(input, bin_sizes):
    concat_list = []
    concat_list.append(input)
    h = input.shape[1]
    for bin_size in bin_sizes:
        pool1 = tf.layers.average_pooling2d(input, (h // bin_size, h // bin_size), (h // bin_size, h // bin_size))
        pool1 = conv2D(pool1, 512, 1)
        pool1 = tf.image.resize_bilinear(pool1, (h, h))
        concat_list.append(pool1)
    result = tf.concat(concat_list, axis=3)
    return result


def pspnet(input, is_training=True):
    ...
    pyd=pyramid_pooling_block(bottom,[1,2,4,8])
